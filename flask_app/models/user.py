...
class User:
    ...
    '''validate create'''
    @staticmethod
    def validate_registration(e):
        is_valid=True
        # 1) declare is valid
        # 1) assign True value
        '''first/last name lengths'''
        # 2) NAME LENGTHS
        if len(e["first_name"]) < 2:
        # 2a) check length
            flash("First name should be greater than 2 characters", "registration")
            # b) set flash message and category
            is_valid=False
            # c) change is_Valid value
        if len(e["last_name"]) < 2:
        # 2a) check length
            flash("Last name should be greater than 2 characters", "registration")
            # b) set flash message and category
            is_valid=False
            # c) change is_valid value
        '''email'''
        ...
        # 3a) compare email to regex format
            flash("Not a valid email", "registration")
            # b) set flash message and category
            is_valid=False
            # c) change is_valid value
        if User.get_email(e):
